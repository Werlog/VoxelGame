...
void SwingMotion::update(float deltaTime, InputHandler& inputHandler, const Player& player)
{
	...
	{
		offset = math::lerp(glm::vec3(0.0f), glm::vec3(0.5f, -0.3f, -0.25f), animationProgress * 2.0f);
		rotation = math::lerp(glm::vec3(0.0f), glm::vec3(0.0f, -15.0f, 60.0f), animationProgress * 2.0f);
	}
	else if (animationProgress > 0.5f)
	{
		offset = math::lerp(glm::vec3(0.5f, -0.3f, -0.25f), glm::vec3(0.0f), (animationProgress - 0.5f) * 2.0f);
		rotation = math::lerp(glm::vec3(0.0f, -15.0f, 60.0f), glm::vec3(0.0f), (animationProgress - 0.5f) * 2.0f);
